...
/* detach a client and terminate the session */
{
    ...
    unlock_subscriber(sub);
	if(gpsd_nmea_sock_fd > 0){
		GPSD_LOG(LOG_INF, &context.errout, "gpsd_nmea_sock_fd:%d destroyed...\n", gpsd_nmea_sock_fd);
		gpsd_nmea_sock_destroy(&gpsd_nmea_sock_fd);
	}
#if 1
	devp = find_device("/dev/ttyHSL1");
	if (devp == NULL) {
		GPSD_LOG(LOG_ERROR, &context.errout, "cant find device!!!\n");
		return;
	}else{
		GPSD_LOG(LOG_INF, &context.errout, "set seen bdgsv false!!\n");
		devp->nmea.seen_bdgsv = false;
	}
#endif
}
...
int set_position_mode(char *buf)
{
	struct gps_device_t *devp;
	...
	devp = find_device("/dev/ttyHSL1");
	if (devp == NULL) {
		GPSD_LOG(LOG_WARN, &context.errout, "cant find device!!!\n");
		return -1;
	}
	if(strstr(buf, "GPS")){
		GPSD_LOG(LOG_WARN, &context.errout, "set gps mode!!!\n");
		gpsd_write(devp, gps_mode, 12);
	}
	else if(strstr(buf, "BDS")){
		GPSD_LOG(LOG_WARN, &context.errout, "set bds mode!!!\n");
		gpsd_write(devp, bds_mode, 12);
	}
	else{
		GPSD_LOG(LOG_WARN, &context.errout, "set gnss mode!!!\n");
		gpsd_write(devp, gnss_mode, 12);
	}

	return 0;
}

int delete_aiding_data(char *buf)
{
	struct gps_device_t *devp = NULL;

	char cold_start[11] = {0xf1, 0xd9, 0x06, 0x40, 0x01, 0x00, 0x01, 0x48, 0x22, 0x0d, 0x0a};
	char warm_start[11] = {0xf1, 0xd9, 0x06, 0x40, 0x01, 0x00, 0x02, 0x49, 0x23, 0x0d, 0x0a};
	char hot_start[11] = {0xf1, 0xd9, 0x06, 0x40, 0x01, 0x00, 0x03, 0x4a, 0x24, 0x0d, 0x0a};
	...
	if(strstr(buf, "COLD")){
		GPSD_LOG(LOG_ERROR, &context.errout, "cold start!!!\n");
		gpsd_write(devp, cold_start, 11);
	}
	else if(strstr(buf, "WARM")){
		GPSD_LOG(LOG_ERROR, &context.errout, "warm start!!!\n");
		gpsd_write(devp, warm_start, 11);
	}
	else{
		GPSD_LOG(LOG_ERROR, &context.errout, "hot start!!!\n");
		gpsd_write(devp, hot_start, 11);
	}
...
static int handle_gpsd_request(struct subscriber_t *sub, const char *buf)
{
	...
	GPSD_LOG(LOG_WARN, &context.errout, "handle_gpsd_request:%s, buf[0]:%c\n", buf, buf[0]);
    reply[0] = '\0';
	if (buf[0] == '?') {
		const char *end;
	...
	{
		if (str_starts_with(buf, "!NMEA_MODE")) {
			GPSD_LOG(LOG_WARN, &context.errout, "will call set position mode: %s\n", buf);
			set_position_mode(buf);
		}else if(str_starts_with(buf, "!NMEA_BOOT")){
			GPSD_LOG(LOG_WARN, &context.errout, "will call delete aiding method: %s\n", buf);
			delete_aiding_data(buf);
		}
	}
					...
					if (client == NULL) {
						...
						if (setsockopt
							...
							(void)throttled_write(client, announce,
												  strlen(announce));
							if(gpsd_nmea_sock_fd < 0){
								gpsd_nmea_sock_fd = gpsd_nmea_sock_init();
								GPSD_LOG(LOG_INF, &context.errout, "gpsd_nmea_sock_fd:%d\n", gpsd_nmea_sock_fd);
							}
						}
				}
