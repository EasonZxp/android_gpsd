...
int gpsd_switch_driver(struct gps_device_t *session, char *type_name)
{
	...
	if(gpsd_nmea_sock_fd < 0){
		...
		GPSD_LOG(LOG_WARN, &session->context->errout, "gpsd_nmea_sock_fd:%d\n", gpsd_nmea_sock_fd);
	}
#endif
...
int gpsd_nmea_sock_destroy(int *sockfd)
{
    close(*sockfd);
    *sockfd = -1;
	return 0;
...
int gpsd_nmea_send(int sockfd, char *buf, int len)
{
	int ret = 0;
...
#if 0
		if (0 == isfinite(fix->speed)) {
			GPSD_LOG(LOG_ERROR, &session->context->errout, "speed_org:%f \n", fix->speed);
			fix->speed = earth_distance(fix->latitude, fix->longitude,
										oldfix->latitude, oldfix->longitude)
				/ deltatime;
			GPSD_LOG(LOG_ERROR, &session->context->errout, "speed_cal:%f \n", fix->speed);
			/* sanity check */
			if (9999.9 < fabs(fix->speed))
				fix->speed = NAN;
		}
